...
const DataFilesLink: React.FC<React.PropsWithChildren<{
  to: string,
}>> = ({
  to,
  children,
        ...
        {data && isOpen && (
          <>
            <div className={`${styles['left-panel']}`}>
              <dl>
                <dt>Execution:</dt><dd>
                  <DataFilesLink to={`./data/browser/${data.archiveSystemId}${data.archiveSystemDir}`}>
                    View in Data Files
                  </DataFilesLink></dd>
                <dt>Output:</dt><dd>
                  <DataFilesLink to={`./data/browser/${data.archiveSystemId}${data.archiveSystemDir}`}>
            ...
            <dl className={`${styles['right-panel']} ${styles['panel-content']}`}>
              <dt>Application</dt>
              <dd>{data.appId} {data.appVersion}</dd>
              <dt>Job ID</dt>
              <dd>{data.uuid}</dd>
              <dt>Status</dt>
...
export const JobsDetailModal: React.FC = () => {
  ...
  return <JobsDetailModalBody uuid={uuid} isOpen={isModalOpen} onToggleModal={handleCancel}/>;
};
