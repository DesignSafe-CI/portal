...
import { Modal, Popover, Select, Spin, Table, Typography, Divider, Row, Col } from 'antd';
...
/* ---------- Helpers  ---------- */
const formatLastFirst = (n?: { first_name?: string; last_name?: string; display_name?: string }) => {
  if (!n) return '';
...
const formatFirstLast = (n?: { first_name?: string; last_name?: string; display_name?: string }) => {
  if (!n) return '';
  const first = (n.first_name || '').trim();
  const last = (n.last_name || '').trim();
...
type ClarivateName = { first_name?: string; last_name?: string; display_name?: string; role?: string };
type ClarivateTitles = { item?: string; source?: string; book_subtitle?: string; series?: string };
type ClarivatePubInfo = {
  pubtype?: string; pubyear?: string | number; pubmonth?: string; vol?: string | number; issue?: string | number;
...
interface YearMonthEntry { yearMonth: string; total: number; }
...
const MetricsModalBody: React.FC<{
...
}> = ({ eventMetricsData, usageMetricsData, clarivateCitations = [], clarivateLoading, clarivateCount }) => {
  /* ---------------- Aggregated Usage ---------------- */
  const citationCount = typeof clarivateCount === 'number'
    ? clarivateCount
  ...
  const totalRequests =
    (eventMetricsData?.data ?? [])
      .filter(d => d?.attributes?.['relation-type-id'] === 'total-dataset-requests-regular')
      .reduce((sum, d) => sum + (d?.attributes?.total ?? 0), 0) || 0;

  /* ---------------- Data by Quarter ---------------- */
  function calculateQuarterSums(yearMonthsData: YearMonthEntry[] = [], year: string) {
  ...
  const mostRecentYear = useMemo(() => {
    ...
    const years = Array.from(new Set(viewsOverTime.map(v => v.yearMonth.slice(0, 4))));
  ...
  useEffect(() => { if (mostRecentYear) setSelectedYear(mostRecentYear); }, [mostRecentYear]);

  const viewsByQ = useMemo(
    () => calculateQuarterSums(usageMetricsData?.data?.attributes?.viewsOverTime || [], selectedYear),
    [usageMetricsData?.data?.attributes?.viewsOverTime, selectedYear]
  );
  const downloadsByQ = useMemo(
    () => calculateQuarterSums(usageMetricsData?.data?.attributes?.downloadsOverTime || [], selectedYear),
    [usageMetricsData?.data?.attributes?.downloadsOverTime, selectedYear]
  );
  const totalsByQ = useMemo(() => {
    return (eventMetricsData?.data ?? []).reduce((acc, curr) => {
      ...
      if (rel === 'total-dataset-requests-regular' && occurredAt) {
        ...
        if (y === selectedYear) {
          const q = `Q${Math.ceil(parseInt(m, 10) / 3)}` as 'Q1' | 'Q2' | 'Q3' | 'Q4';
          acc[q] = (acc[q] || 0) + (curr?.attributes?.total ?? 0);
        }
      }
      return acc;
    }, { Q1: 0, Q2: 0, Q3: 0, Q4: 0 } as Record<'Q1'|'Q2'|'Q3'|'Q4', number>);
  }, [eventMetricsData, selectedYear]);

  const quartersData = [
    { key: 'Q1', quarters: 'Jan-Mar', uniqueInvestigations: viewsByQ.Q1 || '--', uniqueRequests: downloadsByQ.Q1 || '--', totalRequests: totalsByQ.Q1 || '--' },
    { key: 'Q2', quarters: 'Apr-Jun', uniqueInvestigations: viewsByQ.Q2 || '--', uniqueRequests: downloadsByQ.Q2 || '--', totalRequests: totalsByQ.Q2 || '--' },
    { key: 'Q3', quarters: 'Jul-Sep', uniqueInvestigations: viewsByQ.Q3 || '--', uniqueRequests: downloadsByQ.Q3 || '--', totalRequests: totalsByQ.Q3 || '--' },
    { key: 'Q4', quarters: 'Oct-Dec', uniqueInvestigations: viewsByQ.Q4 || '--', uniqueRequests: downloadsByQ.Q4 || '--', totalRequests: totalsByQ.Q4 || '--' },
  ];

  const yearsSelect = useMemo(() => {
    const viewsOverTime = usageMetricsData?.data?.attributes?.viewsOverTime || [];
    const uniqueYears = Array.from(new Set(viewsOverTime.map(v => v.yearMonth.slice(0, 4))));
    return uniqueYears.sort().map(y => <Option key={y} value={y}>{y}</Option>);
  }, [usageMetricsData?.data?.attributes?.viewsOverTime]);
  ...
  return (
    <div>
      <Title level={5} style={{ marginBottom: 8 }}>Aggregated Usage</Title>
      <Row gutter={[16, 16]}>
        <Col xs={24} md={8}>
          <div>
        ...
        <Col xs={24} md={8}>
          <div>
            ...
            <div style={{ marginTop: 4 }}>
              <span style={{ fontSize: 12, fontStyle: 'italic' }}>(downloads)</span>{' '}
              <Popover
                overlayStyle={{ maxWidth: 420 }}
      ...
      <Title level={5} style={{ marginBottom: 8 }}>Data by Quarter</Title>
      <Table dataSource={quartersData} columns={quartersColumns} pagination={false} size="small" />
      ...
      <div style={{ marginTop: 12, textAlign: 'center' }}>
        ...
        <Text>
          ...
          <a href="https://makedatacount.org/" target="_blank" rel="noopener noreferrer">
            Make Data Count
          </a>{' '}
          standard.
        </Text>
        ...
        <div style={{ maxHeight: 400, overflowY: 'auto', paddingRight: 8 }}>
          <ol style={{ paddingLeft: 18, marginBottom: 0 }}>
            {clarivateCitations.map((c, idx) => (
              <li key={c?.doi || idx} style={{ marginBottom: 8, lineHeight: 1.4 }}>
...
export const MetricsModal: React.FC<{
  ...
  const latestYearMonth = useMemo(() => {
    ...
    const mostRecent = [...views].sort((a, b) => b.yearMonth.localeCompare(a.yearMonth))[0].yearMonth; 
  ...
  return (
    <Modal title={title} open={isOpen} onCancel={handleCancel} footer={null} width={800} styles={{ body: { maxHeight: 550, overflowY: 'auto' } }}>
      {ready ? (
        <MetricsModalBody
          eventMetricsData={dataciteEvents as EventMetrics}
          usageMetricsData={dataciteMetrics as UsageMetrics}
          clarivateCitations={clarivateMetrics?.citations as ClarivateCitation[] | undefined}
