...
def neeslisting(offset=0, limit=100, limit_fields=True, *args):
    ...
    if limit_fields:
        pub_query = pub_query.source(
            includes=["project", "pis", "title", "startDate", "path", "description"]
        )
    pub_query = pub_query.sort({"created": {"order": "desc", "unmapped_type": "long"}})
    res = pub_query.execute()
...
def neessearch(offset=0, limit=100, query_string="", limit_fields=True, *args):
    ...
    nees_query_string_query = Q(
        "query_string", query=query_string, default_operator="and"
    )
    client = new_es_client()
    pub_query = IndexedPublicationLegacy.search(using=client).filter(
...
def neesdescription(project_id, *args):
    pub_query = (
        IndexedPublicationLegacy.search()
...
def initilize_publication(
    ...
    if revision:
        base_pub = IndexedPublication.from_id(
            publication["projectId"], revision=None, using=es_client
        )
    ...
    try:
        pub = IndexedPublication.from_id(
            publication["projectId"], revision=revision, using=es_client
        )
