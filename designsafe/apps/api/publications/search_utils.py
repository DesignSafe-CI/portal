            ...
            "nested": {
                ...
                "query": {
                    "nested": {
                        ...
                        "query": {
                            "term": {
                                "experimentsList.value.experimentalFacility._exact": facility[
                                    "name"
                                ]
                            }
                        },
...
def simulation_facility_query(facility):
    # if facility['name'] == 'Other':
    #    facility['name'] = 'other'
    id_query = Q({"match": {"simulations.value.facility.id": facility}})
...
def other_facility_query(facility):
    id_query = Q({"term": {"project.value.facilities.id.keyword": facility}})
    return id_query
...
def other_type_query(data_type):
    ...
    if data_type == "Other":
        return ~Q(
            {"terms": {"project.value.dataType.keyword": data_types}}
        )  # | ~Q({'exists', {'field': 'project.value.dataType.keyword'}})
...
def author_query(author):
    ...
    aq2 = Q(
        "query_string", type="cross_fields", query=author, fields=other_author_fields
    )
