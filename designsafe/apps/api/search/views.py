...
class SearchView(BaseApiView):
    ...
    def get(self, request):
        ...
        doc_type_map = {
            list(
                Index(settings.ES_INDEX_PREFIX.format("publications"))
                .get_alias()
                .keys()
            )[0]: "publication",
            list(
            ...
            list(Index(settings.ES_INDEX_PREFIX.format("files")).get_alias().keys())[
                0
            ]: "file",
            list(Index(settings.ES_INDEX_PREFIX.format("cms")).get_alias().keys())[
                0
            ]: "modelresult",
        }

        public_files_query = (
            CommunityDataSearchManager(request).construct_query()
            | PublishedDataSearchManager(request).construct_query()
        )
        ...
        if type_filter == "published_files":
            ...
            es_query = (
                Search()
                .query(public_files_query | publications_query | cms_query)
                .highlight("body", fragment_size=100)
                .highlight_options(
                    pre_tags=["<b>"], post_tags=["</b>"], require_field_match=False
                )
        ...
        out["hits"] = hits
        out["all_total"] = (
            Search().query(public_files_query | publications_query | cms_query).count()
        )
