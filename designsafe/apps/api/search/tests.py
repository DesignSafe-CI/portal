...
class SearchViewTests(TestCase):
    ...
    def test_search_view(
        ...
        mock_index().get_alias().keys.side_effect = [
            ["test-pub"],
            ["test-pub-legacy"],
            ["test-files"],
            ["test-cms"],
        ...
        mock_search().query().highlight().highlight_options().execute().hits.total.value = (
            0
        )
        url = "{}?type_filter=all&query_string=test".format(
            reverse("designsafe_api:ds_search_api:search")
        )
