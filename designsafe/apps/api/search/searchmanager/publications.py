...
class PublicationsSearchManager(BaseSearchManager):
    ...
    def __init__(self, request=None, **kwargs):
        if request:
            ...
            self.is_filter_selected = reduce(
                lambda x, y: x or y, self.query_dict["typeFilters"].values()
            )

        else:
            self.query_string = kwargs.get("query_string").replace("/", "\\/")

        super(PublicationsSearchManager, self).__init__(IndexedPublication, Search())
    ...
    def simulation_type_query(self, simulation_type):
        return Q(
            {"term": {"simulations.value.simulationType.keyword": simulation_type}}
        )
                ...
                "nested": {
                    ...
                    "query": {
                        "nested": {
                            "path": "project.value",
                            "query": {
                                "term": {"project.value.facility._exact": facility_name}
                            },
                        }
                    },
                }
    ...
    def hybrid_sim_type_query(self, sim_type):
        return Q(
            {"term": {"hybrid_simulations.value.simulationType.keyword": sim_type}}
        )

    def experiment_query(self):
        facility_name = self.query_dict["advancedFilters"]["experimental"][
            "experimentalFacility"
        ]
        experiment_type = self.query_dict["advancedFilters"]["experimental"][
            "experimentType"
        ]
        # Only apply
        if not self.query_dict["typeFilters"]["experimental"] and not (
            facility_name or experiment_type
        ):
            return None
    ...
    def simulation_query(self):
        simulation_type = self.query_dict["advancedFilters"]["simulation"][
            "simulationType"
        ]
    ...
    def field_recon_query(self):
        ...
        nh_event = self.query_dict["advancedFilters"]["field_recon"][
            "naturalHazardEvent"
        ]
        facility_name = self.query_dict["advancedFilters"]["field_recon"]["facility"]
        if facility_name:
            expt_query = expt_query & self.facility_query(facility_name)
        if not self.query_dict["typeFilters"]["field_recon"] and not (
            nh_type or nh_event
        ):
            return None
    ...
    def hybrid_sim_query(self):
        sim_type = data_type = self.query_dict["advancedFilters"]["hybrid_simulation"][
            "hybridSimulationType"
        ]
        facility_name = self.query_dict["advancedFilters"]["hybrid_simulation"][
            "facility"
        ]
        if facility_name:
            expt_query = expt_query & self.facility_query(facility_name)
    ...
    def description_query(self):
        ...
        if not description:
            return None
        return Q(
            "query_string", query=description, fields=["project.value.description"]
        )
    ...
    def listing(self, system=None, file_path=None, offset=0, limit=100, **kwargs):
        ...
        listing_search = listing_search.filter(query).sort(
            "_index", {"created": {"order": "desc", "unmapped_type": "long"}}
        )
        listing_search = listing_search.extra(from_=offset, size=limit).source(
            includes=["project.value", "created", "projectId", "users", "system"]
        )
