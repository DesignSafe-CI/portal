...
class DataFilesView(BaseApiView):
    def get(self, request, api, operation=None, scheme="private", system=None, path=""):
        ...
        if request.user.is_authenticated:
        ...
        elif api in ("agave", "tapis") and system in (
            settings.COMMUNITY_SYSTEM,
            ...
            return JsonResponse(
                {"message": "Please log in to access this feature."}, status=403
            )
    ...
    def put(
        self, request, api, operation=None, scheme="private", system=None, path="/"
    ):
        ...
        try:
            ...
            response = datafiles_put_handler(
                api,
    ...
    def post(
        ...
        response = datafiles_post_handler(
            api,
            request.user.username,
            client,
            scheme,
...
class TransferFilesView(BaseApiView):
    def put(self, request, format):
        ...
        notify(
            request.user.username, "transfer", "Copy operation has started.", "INFO", {}
        )
        try:
            if format == "folder":
