...
def query_file_meta(system, path):
    ...
    while True:
        result = sa_client.meta.listMetadata(
            q=json.dumps(query), limit=300, offset=offset
        )
...
def create_or_update_meta(client, meta_body):
    existing_meta = query_file_meta(
        meta_body["value"]["system"], meta_body["value"]["path"]
    )
...
def update_meta(src_system, src_path, dest_system, dest_path):
    ...
    if meta_listing:
        for meta in meta_listing:
            ...
            new_path = meta_dict["value"]["path"].replace(
                src_path.rstrip("/"), dest_path, 1
            )
        ...
        for update in updates:
            sa_client.meta.updateMetadata(
                uuid=update["uuid"], body=json.dumps(update["body"])
            )
...
def create_meta(path, system, meta):
    ...
    """
    sa_client = service_account()
    meta_body = file_meta_obj(path=path, system=system, meta=meta)
