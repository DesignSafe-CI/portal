...
def listing(client: BoxClient, system, path, offset=0, limit=100, *args, **kwargs):
    offset = int(offset)
    limit = int(limit)
    ...
    items = client.folders.get_folder_items(
        ...
        limit=limit,
        offset=offset,
        fields=["name", "size", "type", "modified_at", "id"],
    )
...
def preview(client: BoxClient, system, path, *args, **kwargs):
    embed_url = client.files.get_file_by_id(path).expiring_embed_link
