...
def listing(client, system, path, offset=None, limit=100, nextPageToken=None, *args, **kwargs):
    ...
    listing = list(map(lambda f: {
        ...
        'format': 'folder' if type(f) == dropbox.files.FolderMetadata else 'raw',
        'mimeType': None,
        'path': f.path_display,
        'name': f.name,
        'length': 0 if type(f) == dropbox.files.FolderMetadata else f.size,
        'lastModified': None if type(f) == dropbox.files.FolderMetadata else f.server_modified,
    }, files.entries))

    reachedEnd = not files.has_more
    nextPageToken = files.cursor if files.has_more else None

    return {'listing': listing, 'nextPageToken': nextPageToken, 'reachedEnd': not files.has_more}
