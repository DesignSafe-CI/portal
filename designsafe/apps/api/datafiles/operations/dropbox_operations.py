    ...
    listing = list(
        map(
            lambda f: {
                "system": "dropbox",
                "type": (
                    "dir" if isinstance(f, dropbox.files.FolderMetadata) else "file"
                ),
                "format": (
                    "folder" if isinstance(f, dropbox.files.FolderMetadata) else "raw"
                ),
                "mimeType": None,
                "path": f.path_display,
                "name": f.name,
                "length": 0 if isinstance(f, dropbox.files.FolderMetadata) else f.size,
                "lastModified": (
                    None
...
def detail(client, system, path, *args, **kwargs):
    ...
    listing_res = {
        "system": "dropbox",
        "type": "dir" if isinstance(f, dropbox.files.FolderMetadata) else "file",
        "format": ("folder" if isinstance(f, dropbox.files.FolderMetadata) else "raw"),
        "mimeType": None,
        "path": f.path_display,
        "name": f.name,
        "length": 0 if isinstance(f, dropbox.files.FolderMetadata) else f.size,
        "lastModified": (
            None if isinstance(f, dropbox.files.FolderMetadata) else f.server_modified
        ),
    }
