...
from designsafe.apps.api.datafiles.operations.agave_operations import (
    preview,
    copy,
    download,
    download_bytes,
...
def listing(client, system, path, username, offset=0, limit=100, *args, **kwargs):
    ...
    search = (
        ...
        .filter(query)
        .sort("name._exact")
        .extra(from_=int(offset), size=int(limit))
    )
    res = search.execute()
...
def search(
    client, system, path, username, offset=0, limit=100, query_string="", **kwargs
):
    """
