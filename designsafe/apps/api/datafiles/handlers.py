...
def datafiles_get_handler(
    api, client, scheme, system, path, operation, username=None, **kwargs
):
...
def datafiles_post_handler(
    api, username, client, scheme, system, path, operation, body=None
):
    ...
    operation in notify_actions and notify(
        username,
        operation,
        "{} operation has started.".format(operation.capitalize()),
        "INFO",
        {},
    )
    ...
    try:
        ...
        operation in notify_actions and notify(
            username,
            operation,
            "{} operation was successful.".format(operation.capitalize()),
            "SUCCESS",
            result,
        )
        return result
        ...
        operation in notify_actions and notify(
            username,
            operation,
            "File operation {} could not be completed.".format(operation.capitalize()),
            "ERROR",
            {},
        )
        raise exc
...
def resource_unconnected_handler(api):
    if api == "googledrive":
        message = (
            'Connect your Google Drive account <a href="'
    ...
    if api == "dropbox":
        message = (
            'Connect your Dropbox account <a href="'
    ...
    if api == "box":
        message = (
            'Connect your Box account <a href="'
...
def resource_expired_handler(api):
    if api == "googledrive":
        message = (
            "While you previously granted this application access to Google Drive, "
            "that grant appears to be no longer valid. Please "
            '<a href="%s">disconnect and reconnect your Google Drive account</a> '
            "to continue using Google Drive data."
            % reverse("googledrive_integration:index")
        )
        return ApiException(status=403, message=message)
    if api == "dropbox":
        message = (
            "While you previously granted this application access to Dropbox, "
            "that grant appears to be no longer valid. Please "
    ...
    if api == "box":
        ...
        message = (
            "While you previously granted this application access to Box, "
            "that grant appears to be no longer valid. Please "
