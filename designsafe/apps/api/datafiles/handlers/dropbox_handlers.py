...
def dropbox_get_handler(client, scheme, system, path, operation, **kwargs):
    if operation not in allowed_actions[scheme]:
        raise PermissionDenied
    op = getattr(dropbox_operations, operation)
    return op(client, system, path, **kwargs)


def dropbox_post_handler(client, scheme, system,
...
def dropbox_put_handler(client, scheme, system,
                      path, operation, body=None):
