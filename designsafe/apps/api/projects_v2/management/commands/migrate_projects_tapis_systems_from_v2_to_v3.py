...
def create_or_update_workspace_system(create, client, system_id: str, title: str, description: str,  # pylint: disable=too-many-arguments
                                      project_root_dir: str , owner=None) -> str:
...
HOST = settings.PROJECT_STORAGE_SYSTEM_TEMPLATE["storage"]["host"]  # cloud.corral.tacc.utexas.edu
PORT = 22
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):  # pylint: disable=too-many-statements disable=too-many-locals
        ...
        TAPIS_TENANT_BASEURL = os.environ.get('TAPIS_TENANT_BASEURL') # pylint: disable=invalid-name
        TAPIS_ADMIN_JWT = os.environ.get('TAPIS_ADMIN_JWT') # pylint: disable=invalid-name
        ...
        for i, project in enumerate(iterate_entities()):
            ...
            description = project["value"]["description"] \
                if "description" in project["value"] else ""
            pi = project['value']['pi'] # pylint: disable=invalid-name
            co_pis = project['value']['coPis'] if 'coPis' in project['value'] else []
            team_members = project['value']['teamMembers'] \
                if 'teamMembers' in project['value'] else []
            guest_members = project['value']['guestMembers'] \
                if ('guestMembers' in project['value']) else []
            ...
            if not dry_run:
                ...
                try:
                    create_or_update_workspace_system(create, client, system_id=system,
