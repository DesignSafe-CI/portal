...
def create_or_update_workspace_system(create, client, system_id: str, title: str, description, projectRootDir,
                                      owner=None) -> str:
    system_args = {
        "id": system_id,
        "host": HOST,
...
HOST = settings.PROJECT_STORAGE_SYSTEM_TEMPLATE["storage"]["host"]  # ie. cloud.corral.tacc.utexas.edu
PORT = 22
TG_USER = settings.PROJECT_STORAGE_SYSTEM_TEMPLATE["storage"]["auth"]["username"]  # i.e. tg458981
TG_USER_PRIVATE_KEY = settings.PROJECT_STORAGE_SYSTEM_TEMPLATE["storage"]["auth"]["privateKey"]
TG_USER_PUBLIC_KEY = settings.PROJECT_STORAGE_SYSTEM_TEMPLATE["storage"]["auth"]["publicKey"]
OWNER = "wma_prtl"


class Command(BaseCommand):
    help = ('Facilitates the migration of project systems from Tapis v2 to v3,'
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--dry-run',
            action='store_true',
            help='Executes the command in a simulation mode, logging actions without applying any changes '
                 'to the systems or data.',
        )

        parser.add_argument(
            '--update-existing',
            action='store_true',
            help='Allows the command to update systems that already exist in Tapis V3, ensuring they are'
                 'synchronized with their V2 counterparts.',
        )
    ...
    def handle(self, *args, **options):
        ...
        total_number_projects = len([p for p in iterate_entities()])

        for i, project in enumerate(iterate_entities()):
            uuid = project["uuid"]
            projectId = project["value"]["projectId"]
            system = f"project-{uuid}"
            projectRootDir = f"/corral-repl/projects/NHERI/projects/{uuid}"
            title = project["value"]["title"]
            description = project["value"]["description"] if "description" in project["value"] else ""
            pi = project['value']['pi']
            coPis = project['value']['coPis'] if 'coPis' in project['value'] else []
            teamMembers = project['value']['teamMembers'] if 'teamMembers' in project['value'] else []
            guestMembers = project['value']['guestMembers'] if 'guestMembers' in project['value'] else []
            guestMembers = [u['user'] for u in guestMembers if u is not None]

            all_writers = coPis + teamMembers + guestMembers
            if pi is not None:
                all_writers.append(pi)

            all_readers = guestMembers
            all_users = all_readers + all_writers

            logger.info(
                f"Migrating {i}/{total_number_projects} {projectId} ({uuid}) ('{title}') pi:{pi} coPis:{coPis} teamMembers:{teamMembers} guestMembers:{guestMembers}")
            ...
            if not dry_run:
                if system_exists and not update_existing:
                    logger.info("System already exists so skipping (update_existing=False)")
                    continue
                if system_exists:
                    shared_information = client.systems.getShareInfo(systemId=system)
                    user_to_remove = set(shared_information.get("users")) - set(all_users)
