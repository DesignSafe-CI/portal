...
def box_download(
    self, username, src_resource, src_file_id, dest_resource, dest_file_id
):
    """
    :param self:
    :param username:
    ...
    logger.debug(
        "Downloading %s://%s for user %s to %s://%s",
        src_resource,
        src_file_id,
        username,
        dest_resource,
    ...
    try:
        target_path = reverse(
            "designsafe_data:data_depot", args=[src_resource, src_file_id]
        )
        n = Notification(
            event_type="data",
        ...
        target_path = reverse(
            "designsafe_data:data_depot", args=[dest_resource, dest_file_id]
        )
        n = Notification(
            event_type="data",
        ...
        logger.exception(
            "Unexpected task failure: box_download",
            extra={
                "username": username,
...
def box_download_folder(box_file_manager, box_folder_id, download_path):
    ...
    logger.debug(
        "Creating directory %s <= box://folder/%s", directory_path, box_folder_id
    )
    try:
...
def box_upload(self, username, src_resource, src_file_id, dest_resource, dest_file_id):
    ...
    """
    logger.debug(
        "Importing file %s://%s for user %s to %s://%s"
    ...
    try:
        ...
        if file_type != "folder":
            ...
            raise Exception(
                "You can only import files to a folder!",
                status=400,
                extra={
                    "src_resource": src_resource,
                    "src_file_id": src_file_id,
        ...
        if src_resource == "agave" or src_resource == "public":
            try:
                ...
                if os.path.isfile(file_real_path):
                    ...
                    logger.error(
                        "Unable to upload %s: file does not exist!", file_real_path
                    )
            except:
        ...
        logger.exception(
            "Unexpected task failure: box_upload",
            extra={
                "username": username,
                "src_resource": src_resource,
                "src_file_id": src_file_id,
...
def box_upload_file(box_file_manager, box_folder_id, file_real_path):
    ...
    with open(file_real_path, "rb") as file_handle:
        ...
        logger.info(
            "Successfully uploaded %s to box:folder/%s as box:file/%s",
            file_real_path,
            box_folder_id,
            uploaded_file.object_id,
        )
...
@shared_task(bind=True)
def copy_public_to_mydata(
    self, username, src_resource, src_file_id, dest_resource, dest_file_id
):
    logger.debug(
        "Scheduled copy of files from %s://%s to %s://%s",
        src_resource,
        src_file_id,
        dest_resource,
        dest_file_id,
    ...
    try:
        n = Notification(
            ...
            message="Copying folder/files %s from public data to your private data. Please wait..."
            % (src_file_id,),
            user=username,
            extra={
                "system": dest_resource,
        ...
        if source_fm_cls and dest_fm_cls:
            ...
            dest_real_path = os.path.join(
                dest_fm.get_file_real_path(dest_file_id), dirname
            )
            ...
            logger.error(
                "Unable to load file managers for both source=%s and destination=%s",
                src_resource,
                dest_resource,
            )
...
def set_project_id(self, project_uuid):
    ...
    for i in range(10):
        _projs = service.meta.listMetadata(
            q='{{"name": "designsafe.project", "value.projectId": {} }}'.format(
                project_id
            )
        )
        if len(_projs):
...
def index_projects_listing(projects):
    ...
    for _project in projects:
        ...
        project_dict = {
            key: value for key, value in project_dict.items() if key != "_links"
        }
...
def freeze_publication_meta(
    self, project_id, entity_uuids=None, revision=None, revised_authors=None
):
    ...
    try:
        PublicationManager.freeze_project_and_entity_metadata(
            project_id, entity_uuids, revision, revised_authors
        )
...
@shared_task(bind=True, max_retries=1, default_retry_delay=60)
def amend_publication_data(
    self, project_id, amendments=None, authors=None, revision=None
):
    """Amend publication.
    ...
    try:
        amended_pub = PublicationManager.amend_publication(
            project_id, amendments, authors, revision
        )
...
def save_publication(
    ...
    try:
        PublicationManager.draft_publication(
            project_id, entity_uuids, revision=revision, revised_authors=revised_authors
        )
...
def swap_file_tag_uuids(self, project_id, revision=None):
    ...
    try:
        ...
        logger.error(
            "File Tag Correction Error: %s. %s", project_id, exc, exc_info=True
        )
        raise self.retry(exc=exc)
...
def set_publish_status(
    self, project_id, entity_uuids=None, publish_dois=False, revision=None
):
    from designsafe.apps.projects.managers import publication as PublicationManager
    ...
    PublicationManager.publish_resource(
        project_id, entity_uuids, publish_dois, revision=revision
    )
...
def email_project_admins(
    self, project_id, project_uuid, project_title, project_url, username
):
...
@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def email_collaborator_added_to_project(
    self,
    project_id,
    project_uuid,
    project_title,
        ...
        if collab_users:
            for collab_user in collab_users:
                email_body = """
                        ...
                        This is a programmatically generated message. Do NOT reply to this message.
                        """.format(
                    name=collab_user.get_full_name(),
                    title=project_title,
                    prjID=project_id,
                    url=project_url,
                )
                try:
                    collab_user.profile.send_mail(
                        "You have been added to a DesignSafe project!", email_body
                    )
...
def email_user_publication_request_confirmation(self, username):
    ...
    """.format(
        pub_url="https://www.designsafe-ci.org/data/browser/public/",
        ticket_url="https://www.designsafe-ci.org/help/new-ticket/",
    )
    try:
...
def check_published_files(self, project_id, revision=None, selected_files=None):
    ...
    if missing_files or empty_folders:
        ...
        logger.info(
            "check_published_files missing files: "
            + project_id
            + " "
            + str(missing_files)
        )
        logger.info(
            "check_published_files empty folders: "
            + project_id
            + " "
            + str(empty_folders)
        )
