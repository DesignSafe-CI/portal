...
UNAMENDABLE_FIELDS = {  # review
    ...
    "subentity": ["project", "experiments", "modelConfigs", "sensorLists", "files"],
}
...
def draft_publication(
        ...
        for ent_uuid in main_entity_uuids:
            ...
            if entity:
                if revision:
                    ...
                    pub_ent = next(
                        ent for ent in original_entities if ent.uuid == ent_uuid
                    )
                    ...
                    entity_url = ENTITY_TARGET_BASE.format(
                        project_id=project_id, entity_uuid=ent_uuid
                    )
                ...
                if upsert_main_entity_doi and main_entity_doi:
                    me_res = DataciteManager.create_or_update_doi(
                        ent_datacite_json, main_entity_doi
                    )
    ...
    if upsert_project_doi and project_doi:
        prj_res = DataciteManager.create_or_update_doi(prj_datacite_json, project_doi)
        ...
        for doi in prj.dois:
            prj_res = DataciteManager.create_or_update_doi(prj_datacite_json, doi)
            responses.append(prj_res)
    ...
    for res in responses:
        logger.info("DOI created or updated: %(doi)s", {"doi": res["data"]["id"]})
    return responses
            ...
            for pub_ent in pub_dict[FIELD_MAP[entity["name"]]]:
                if pub_ent["uuid"] == entity["uuid"]:
                    for key in entity["value"]:
                        ent_type = (
                            "entity" if "dois" in entity["value"] else "subentity"
                        )
...
def amend_datacite_doi(publication):
    ...
    if project_type == "other":
        ...
        project = prj_class(
            value=pub_dict["project"]["value"], uuid=pub_dict["project"]["uuid"]
        )
        ...
        for field in publication.entity_keys(publishable=True):
            for ent in pub_dict[field]:
                ...
                entity.authors = ent[
                    "authors"
                ]  # swap author formats before creating json
                ent_datacite_json = entity.to_datacite_json(prj)
...
def publish_resource(project_id, entity_uuids=None, publish_dois=False, revision=None):
    ...
    for res in responses:
        logger.info("DOI published: %(doi)s", {"doi": res["data"]["id"]})
    return responses
...
def _set_related_entities(entity, publication):
    ...
    for attr in reverse_field_attrs:
        try:
            ...
            logger.exception(
                "No field '%(attr)s' in '%(ent)s'", {"attr": attr, "ent": entity}
            )
...
def _set_authors(entity, publication):
    ...
    for author in publication["authors"]:
        ...
        try:
            ...
            logger.exception(
                "Error retrieving user: %(user)s", {"user": author.get("name")}
            )
        if not user:
            continue
...
def fix_file_tags(project_id, revision=None):
    ...
    entities_to_check = list(
        set(pub_dict.keys()).intersection(list(FIELD_MAP.values()))
    )
    ...
    def fix_tags_no_path(entity):
        if entity["name"] == "designsafe.project":
            proj_other = BaseFileResource.listing(
                service_account(), system="project-{}".format(entity["uuid"]), path=""
            )
            for child in proj_other.children:
                try:
                    ...
                    pub_file = BaseFileResource.listing(
                        service_account(),
                        system="designsafe.storage.published",
                        path="{}{}".format(pub_base, child.path),
                    )
                    proj_file = BaseFileResource.listing(
                        service_account(),
                        system="project-{}".format(entity["uuid"]),
                        path=child.path,
                    )
                    for tag in entity["value"]["fileTags"]:
            ...
            for fobj in entity["fileObjs"]:
                try:
                    ...
                    pub_file = BaseFileResource.listing(
                        service_account(),
                        system="designsafe.storage.published",
                        path="{}{}".format(pub_base, fobj["path"]),
                    )
                    proj_file = BaseFileResource.listing(
                        service_account(),
    ...
    for entname in entities_to_check:
        if type(pub_dict[entname]) == list:
            for entity in pub_dict[entname]:
                if (
                    "value" in entity
                    and "fileTags" in entity["value"]
                    and check_complete_tags(entity["value"]["fileTags"])
                ):
            ...
            if (
                "value" in pub_dict[entname]
                and "fileTags" in pub_dict[entname]["value"]
                and check_complete_tags(pub_dict[entname]["value"]["fileTags"])
            ):
        ...
        try:
            ...
            for dirs, _, files in os.walk(arc_source):
                for f in files:
                    ...
                    zf.write(
                        os.path.join(dirs, f),
                        os.path.join(dirs.replace(pub_dir, ""), f),
                    )
...
def freeze_project_and_entity_metadata(
    project_id, entity_uuids=None, revision=None, revised_authors=None
):
        ...
        for ent_uuid in entity_uuids:
            ...
            if entity:
                ...
                if entity_dict["value"]["dois"]:
                    ...
                    if revision:
                        _preserve_project_values(
                            original_pub, entity_dict, revised_authors
                        )
