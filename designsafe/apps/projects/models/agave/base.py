...
class RelatedEntity(MetadataModel):
    ...
    def to_datacite_json(self):
        ...
        authors = [
            author
            for author in getattr(self, "authors", [])
        ...
        attributes["titles"] = [{"title": self.title}]
        ...
        attributes["subjects"] = [{"subject": entity.title} for entity in entities]
        return attributes
...
class Project(MetadataModel):
    ...
    def collaborators(self):
        return {"pi": self.pi, "coPis": self.co_pis, "teamMembers": self.team_members}
    ...
    def remove_team_members(self, usernames):
        self._remove_team_members_pems(usernames)
        self.team_members = [
            member for member in self.team_members if member not in usernames
        ]
        self.save(self.manager().agave_client)
        return self
    ...
    def _add_team_members_pems(self, usernames):
        ...
        for username in usernames:
            ...
            agave_client.systems.updateRole(
                systemId=self.system, body={"username": username, "role": "USER"}
            )
        return self
    ...
    def _remove_team_members_pems(self, usernames):
        ...
        for username in usernames:
            ...
            agave_client.systems.updateRole(
                systemId=self.system, body={"username": username, "role": "NONE"}
            )
        return self
    ...
    def related_entities(self, offset=0, limit=100):
        ...
        rel_names = [
            getattr(self, attrname).related_obj_name
            for attrname in relattrs
    ...
    def to_dataset_json(self, **kwargs):
        ...
        dataset_json = {
            ...
            "@type": "Dataset",
            "url": "https://www.designsafe-ci.org/data/browser/public/designsafe.storage.published/"
            + self.project_id,
            "identifier": {
                "@id": "",
                "@type": "PropertyValue",
            ...
            "author": [{"name": "", "affiliation": "", "@id": "", "identifier": ""}],
            "datePublished": self.created,
            "dateModified": self.to_body_dict()["lastUpdated"],
            "description": self.description,
            "keywords": self.keywords.split(","),
            "license": {"@type": "CreativeWork", "license": "", "url": ""},
            "publisher": {
                "@type": "Organization",
                "name": "Designsafe-CI",
            ...
            for i in range(len(related_ents)):
                if hasattr(related_ents[i], "dois") and related_ents[i].dois:
                    ...
                    if getattr(related_ents[i], "team_order", False):
                        authors = sorted(
                            related_ents[i].team_order, key=lambda x: x["order"]
                        )
                    else:
                        authors = [
                            {"name": username} for username in [self.pi] + self.co_pis
                        ]
                    ...
                    dataset_json["hasPart"].append(
                        {
                            ...
                            "dateModified": related_ents[i].to_body_dict()[
                                "lastUpdated"
                            ],
                            ...
                            "funding": (
                                generate_awards(awards[i]) if i < len(awards) else ""
                            ),
                        }
                    )
    ...
    def to_datacite_json(self, project=None):
        ...
        if len(self.award_number) and type(self.award_number[0]) is not dict:
            self.award_number = [{"order": 0, "name": "".join(self.award_number)}]
        awards = sorted(
            self.award_number, key=lambda x: (x.get("order", 0), x.get("name", ""))
        )
        ...
        for related_entity in self.related_entities():
            ...
            if "dois" in rel_ent_dict["value"] and len(rel_ent_dict["value"]["dois"]):
                ...
                attributes["relatedIdentifiers"].append(
                    {
                        "relatedIdentifier": "https://agave.designsafe-ci.org/meta/v2/data/"
                        + related_entity["value"]["uuid"],
    ...
    for author in authors:
        ...
        if not author.get("guest"):
            try:
                ...
                user_orcid = (
                    user_obj.profile.orcid_id if user_obj.profile.orcid_id else None
                )
        ...
        if user_orcid:
            details = {"@id": user_orcid, "identifier": user_orcid}
        else:
            details = {}
