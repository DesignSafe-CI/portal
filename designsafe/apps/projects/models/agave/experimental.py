...
class Experiment(RelatedEntity):
    ...
    experiment_type = fields.CharField(
        "Experiment Type", max_length=255, default="other"
    )
    experiment_type_other = fields.CharField(
        "Experiment Type Other", max_length=255, default=""
    )
    description = fields.CharField("Description", max_length=1024, default="")
    ...
    def to_datacite_json(self, project=None):
        ...
        if (
            hasattr(self, "experimental_facility")
            and len(self.experimental_facility)
            and ("None" not in self.experimental_facility)
        ):
        ...
        if project:
            attributes["titles"] = attributes.get("titles", []) + [
                {"titleType": "Subtitle", "title": project.title}
            ]
            ...
            if len(project.award_number) and type(project.award_number[0]) is not dict:
                project.award_number = [
                    {"order": 0, "name": "".join(project.award_number)}
                ]
            awards = sorted(
                project.award_number,
...
class SensorList(RelatedEntity):
    ...
    sensor_list_type = fields.CharField(
        "Sensor List Type", max_length=255, default="other"
    )
    title = fields.CharField("Title", max_length=1024)
    description = fields.CharField("Description", max_length=1024, default="")
    project = fields.RelatedObjectField(ExperimentalProject)
