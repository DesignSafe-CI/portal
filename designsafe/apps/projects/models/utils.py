...
def lookup_model(entity=None, name=None):
    ...
    1. Get entity name from json object or :code:`name` string.
    2. Remove :code:`designsafe.project` from the :code:`name` since it's a commonality.
    3. Get project type from the resulting :code:`name` since :code:`name`'s are in the form of
       :code:`designsafe.project.<project_type>.<entity_name>`.
    4. Get the python modules where we'll search the model class.

        4.1 If :code:`name` is :code:`"designsafe.project"` we are looking at a project and we create
            a string of the form :code:`<project_type>_project` and add every project model class
            to our search list.
    ...
    5. Transform the entity name from spinal case to reverse camel case. e.g.
       from :code:`"simulation_project"` to :code:`"SimulationProject"`. or
       from :code:`"model_config"` to :code:`"ModelConfig"`.
    6. Loop through the search list and try to load the class using the transformed
       string in step 5.
    7. If nothing was found then return :class:`~designsafe.apps.projects.models.agave.base.Project`.
