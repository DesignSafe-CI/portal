...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("output", help="Output filepath", default="report.csv")
    ...
    def handle(self, *args, **options):
        ...
        with open(filepath, "wb") as _file:
            ...
            for project in self.get_projects():
                ...
                co_pis_str = ", ".join([self.user_full_name(co_pi) for co_pi in co_pis])
                row.append(co_pis_str)
                team_members = project.value.get(
                    "teamMember", project.value.get("teamMembers", ["-"])
                )
                team_members_str = ", ".join(
                    [self.user_full_name(team_member) for team_member in team_members]
                )
