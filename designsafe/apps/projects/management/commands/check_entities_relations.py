...
class Command(BaseCommand):
    ...
    def _delete_relations(self, ent, uuids):
        ...
        ent.association_ids = [
            uuid for uuid in ent.association_ids if uuid not in uuids
        ]
    ...
    def handle(self, *args, **options):
        ...
        entities = self.client.meta.listMetadata(
            q=json.dumps({"name": {"$in": rel_names}, "associationIds": prj.uuid})
        )
