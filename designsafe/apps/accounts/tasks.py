...
def create_report(username, list_name):
    ...
    try:
        ...
        logger.info(
            "Creating user report for user=%s on " "default storage systemId=%s",
            username,
        ...
        for user in user_list:
            if user.username == "EF-UF":
                writer.writerow(
                    [
                        'Unable to find user data for username "' + user.username + '"',
                    ]
                )
                continue
            try:
                ...
                if hasattr(designsafe_user, "profile"):
                    ...
                    interests = designsafe_user.profile.nh_interests.all().values(
                        "description"
                    )
                    ...
                    activities = (
                        designsafe_user.profile.research_activities.all().values(
                            "description"
                        )
                    )
                    research_activities = [
                    ...
                    writer.writerow(
                        [
                            (
                                ...
                                else user_profile.lastName
                            ),
                            (
                                user_profile.firstName
                                if user_profile.firstName
                                else user_profile.firstName
                            ),
                            user_profile.email,
                            user_profile.phone,
                            ...
                            (
                                ...
                                if designsafe_user.profile.bio
                                else designsafe_user.profile.bio
                            ),
        ...
        logger.exception(
            ...
            extra={"user": username, "systemId": settings.AGAVE_STORAGE_SYSTEM},
        )
