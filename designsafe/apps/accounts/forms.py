...
def get_institution_choices():
    ...
    return (("", "Choose one"),) + tuple(
        (c["id"], c["name"]) for c in institutions_list
    )
...
class EmailConfirmationForm(forms.Form):
    ...
    password = forms.CharField(
        widget=forms.PasswordInput, label="Enter Your TACC Password", required=True
    )
...
class ChangePasswordForm(forms.Form):
    ...
    def clean(self):
        ...
        tas = TASClient()
        current_password_correct = tas.authenticate(
            self._username, cleaned_data["current_password"]
        )
...
class ProfessionalProfileForm(forms.ModelForm):
    ...
    professional_level = forms.ChoiceField(
        choices=PROFESSIONAL_LEVEL_OPTIONS, widget=forms.RadioSelect, required=True
    )
...
class UserRegistrationForm(UserProfileForm, ProfessionalProfileForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["institutionId"].choices += (
            ("-1", "My Institution is not listed"),
        )
    ...
    def clean(self):
        ...
        if (
            ...
            valid, error_message = check_password_policy(
                self.cleaned_data, password, confirmPassword
            )
            if not valid:
                self.add_error("password", error_message)
                self.add_error("confirmPassword", "")
    ...
    def save(self, source="DesignSafe", pi_eligibility=INELIGIBLE):
        ...
        try:
            ...
            LOGGER.warning(
                "On TAS registration, local user already existed? " "user=%s" % user
            )
...
class NEESAccountMigrationForm(forms.Form):
    ...
    email_address = forms.EmailField(
        label=_("Your NEEShub Email Address"),
        help_text=_("Enter the email address associated " "with your NEEShub account."),
    )
