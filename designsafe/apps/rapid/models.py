...
class RapidNHEventType(Document):
    ...
    display_name = Text(fields={"_exact": Keyword()})
...
class RapidNHEvent(Document):
    ...
    event_type = Text(fields={"_exact": Keyword()})
    location_description = Text(analyzer="english", fields={"_exact": Keyword()})
    location = GeoPoint()
    main_image_url = Text(fields={"_exact": Keyword()})
    main_image_uuid = Text(fields={"_exact": Keyword()})
    datasets = Nested(
        properties={
            "id": Text(fields={"_exact": Keyword()}),
