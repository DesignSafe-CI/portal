...
class FileMediaView(View):
    ...
    def get(self, request, file_mgr_name, system_id, file_path):
        if (
            file_mgr_name not in ["public", "community"]
        ...
        filepath = "{corral}/{sys_dirname}/{file_path}".format(
            corral=self.corral,
            sys_dirname=self.get_system_dirname(system_id),
            file_path=file_path,
        )
        ...
        response["X-Accel-Redirect"] = "/internal-resource/{filepath}".format(
            filepath=filepath
        )
        return response
...
class DataDepotPublishedView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if (
            pub.project.value.to_dict().get("dois") != None
        ):  # This is for newer publications
            ...
            context["authors"] = [
                {
                    ...
                    "institution": getattr(
                        getattr(user, "profile", ""), "institution", ""
                    ),
                }
...
class DataDepotLegacyPublishedView(TemplateView):
    ...
    @method_decorator(ensure_csrf_cookie)
    def dispatch(self, request, *args, **kwargs):
        return super(DataDepotLegacyPublishedView, self).dispatch(
            request, *args, **kwargs
        )
