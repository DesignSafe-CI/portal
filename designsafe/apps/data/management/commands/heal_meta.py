...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if base_path:
            ...
            self.stdout.write('Indexing permissions only.')
            mgr.index_permissions(system_id, base_path, username, levels = levels, bottom_up = False)
        elif options['fullindex']:
            self.stdout.write('Full Indexing.')
            mgr.index(system_id, base_path, username, full_indexing = True, levels = levels)
        else:
            self.stdout.write('Normal Indexing.')
