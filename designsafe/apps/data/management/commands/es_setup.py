...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--username", help="Username to populate files index", default="envision"
        )
        parser.add_argument(
            "--remote",
            help="Remote role to index from, either staging or default",
            default="staging",
        )
        parser.add_argument(
            "--local",
    ...
    def handle(self, *args, **options):
        ...
        local_es_client = elasticsearch.Elasticsearch(
            settings.ES_CONNECTIONS[local]["hosts"]
        )
        remote_es_client = elasticsearch.Elasticsearch(
            settings.ES_CONNECTIONS[remote]["hosts"],
