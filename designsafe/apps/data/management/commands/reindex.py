...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--remote-host",
            help="Remote host where 'from_index' resides."
        ...
        parser.add_argument(
            "--sample",
            help="set to True to take a random sample",
            default=False,
            type=bool,
        )
        parser.add_argument(
            "--size",
            help="size of the sample (default=100000)",
            default=100000,
            type=int,
        )
        parser.add_argument(
            "--seed", help="seed for randomization", default="tacc rulz ok", type=str
        )
    ...
    def handle(self, *args, **options):
        ...
        body = {"source": {"index": from_index}, "dest": {"index": to_index}}
        ...
        if remote_host:
            if remote_host.startswith("http://") or remote_host.startswith("https://"):
                hosts = [remote_host]
        ...
        if not hosts:
            raise CommandError(
                "No valid remote hosts given. Remote host value given: {}".format(
                    remote_host
                )
            )
