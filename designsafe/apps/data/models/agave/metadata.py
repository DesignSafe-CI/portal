        ...
        if self.uuid is None:
            ...
            logger.info(
                'Updating "{}" metadata {}: {}'.format(
                    self.name, self.uuid, self.request_body
                )
            )
            result = self._agave.meta.updateMetadata(
                uuid=self.uuid, body=self.request_body
            )
        self._wrapped.update(**result)
        return self
...
class BaseMetadataPermissionResource(BaseAgaveResource):
    ...
    def __init__(self, metadata_uuid, agave_client, **kwargs):
        defaults = {"permission": {}, "username": None}
        defaults.update(**kwargs)
    ...
    def request_body(self):
        return json.dumps(
            {"username": self.username, "permission": self.permission_bit}
        )
    ...
    def save(self):
        ...
        """
        logger.info(
            "Updating metadata permissions: {} {}".format(
                self.metadata_uuid, self.request_body
            )
        )
        result = self._agave.meta.updateMetadataPermissions(
            uuid=self.metadata_uuid, body=self.request_body
        )
        self._wrapped.update(**result)
        return self
