...
class Manager(object):
    ...
    def list(self, agave_client, association_id=None):
        if association_id is None:
            metas = agave_client.meta.listMetadata(
                q=json.dumps({"name": self.model_cls.model_name})
            )
        else:
            metas = agave_client.meta.listMetadata(
                q=json.dumps(
                    {
                        "name": self.model_cls.model_name,
...
class BaseModel(type):
    ...
    def _prepare(cls):
        ...
        if cls.__doc__ is None:
            cls.__doc__ = "%s(%s)" % (
                cls.__name__,
                ", ".join(f.name for f in opts._fields),
            )
...
class Model(object, metaclass=BaseModel):
    ...
    def permissions(self):
        if not self._permissions:
            self._permissions = (
                self.manager().agave_client.meta.listMetadataPermissions(uuid=self.uuid)
            )
    ...
    def permissions(self, val):
        ...
        if (
            not val["permission"].get("read", False)
            and not val["permission"].get("write", False)
            and not val["permission"].get("execute", False)
        ):
    ...
    def to_body_dict(self):
        ...
        if not self._is_nested:
            ...
            [
                ...
                for v in self.association_ids
                if v not in deduped_list
            ]
        ...
        if "lastUpdated" in dict_obj and isinstance(
            dict_obj["lastUpdated"], datetime.datetime
        ):
