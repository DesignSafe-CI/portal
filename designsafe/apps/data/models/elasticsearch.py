...
class IndexedFile(Document):
    ...
    _links = Object(
        properties={
            "self": Object(properties={"href": Keyword()}),
            "system": Object(properties={"href": Keyword()}),
        }
    )
...
class IndexedPublication(Document):
        ...
        properties={
            ...
            "value": Nested(
                properties={
                    ...
                    "description": Text(analyzer="english"),
                    "equipmentType": Text(
                        analyzer="english", fields={"_exact": Keyword()}
                    ),
                    "equipmentTypeOther": Text(
                        analyzer="english", fields={"_exact": Keyword()}
                    ),
                    "experimentalFacility": Text(
                        analyzer="english", fields={"_exact": Keyword()}
                    ),
                    "experimentalFacilityOther": Text(
                        analyzer="english", fields={"_exact": Keyword()}
                    ),
                    "title": Text(analyzer="english"),
                    "project": Text(fields={"_exact": Keyword()}),
    ...
    project = Object(
        properties={
            ...
            "value": Object(
                properties={
                    "assocaitedProjects": Nested(
                        properties={
                            "href": Text(fields={"_exact": Keyword()}),
                            "title": Text(
                                fields={"_exact": Keyword()}, analyzer="english"
                            ),
                        }
                    ),
    ...
    def from_id(cls, project_id, revision=None, using="default"):
        ...
        if res.hits.total.value > 1:
            ...
            raise DocumentNotFound("No document found for " "{}".format(project_id))
...
class IndexedPublicationLegacy(Document):
    ...
    project = Text(fields={"_exact": Keyword()})
    sponsor = Nested(properties={"name": Text(analyzer="english"), "url": Text()})
    ...
    experiments = Nested(
        properties={
            ...
            "deleted": Boolean(),
            "path": Text(
                fields={"_exact": Keyword(), "_path": Text(analyzer=path_analyzer)}
            ),
            "material": Nested(
                properties={
