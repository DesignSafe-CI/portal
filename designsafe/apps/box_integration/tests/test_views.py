...
class BoxInitializationTestCase(TestCase):
    ...
    def test_oauth2_callback(self, m_box_oauth_authenticate, m_user_get):
        ...
        response = self.client.get(
            reverse("box_integration:oauth2_callback"),
            {"code": "box_authorization_code_123", "state": "box_csrf_state_123"},
        )
        ...
        self.assertRedirects(
            response, reverse("box_integration:index"), fetch_redirect_response=False
        )

        response = self.client.get(reverse("box_integration:index"))
        self.assertContains(response, "Box.com Enabled")
        self.assertContains(
            response, "Box.com as <em>DS User (ds_user@designsafe-ci.org)</em>"
        )
...
class BoxDisconnectTestCase(TestCase):
    ...
    def test_disconnect(self):
        ...
        self.assertRaises(
            BoxUserToken.DoesNotExist, BoxUserToken.objects.get, user=user
        )
