...
app.requires.push('ds.wsBus', 'ds.notifications', 'logging', 'toastr', 'ui.bootstrap');
        ...
        NotificationService.list(params).then(function(resp) {
            ...
            // $scope.data.notifications[i]['datetime'] = Date($scope.data.notifications[i]['datetime']);
                ...
                if ($scope.data.notifications[i]['event_type'] == 'job') {
                    $scope.data.notifications[i]['action_link']= '/rw/workspace/api/notification/process' + $scope.data.notifications[i]['pk'];
                } else if ($scope.data.notifications[i]['event_type'] == 'data_depot') {
                    $scope.data.notifications[i]['action_link']= '/api/notifications/' + $scope.data.notifications[i]['pk'];
                }
            }
