...
def walk_hyb_sim(project_id, version=None):
    ...
    for hyb_sim in hybrid_sim_list:
        ...
        hyb_sim_container_path = "{}/{}".format(
            project_id, parse.quote(hyb_sim.value.title)
        )
        print("hybrid sim " + hyb_sim.value.title)
        hyb_sim_doi = hyb_sim.doi
        project_map["fedora_mapping"]["generated"].append(
            "Hybrid Simulation: {}".format(hyb_sim_doi)
        )
        ...
        for report in reports_list:
            ...
            report_container_path = "{}/{}".format(
                project_id, parse.quote(report.value.title)
            )
            ...
            report_map = {
                ...
                "fedora_mapping": {
                    **format_report(report),
                    "wasGeneratedBy": "Hybrid Simulation: {}".format(hyb_sim_doi),
                },
        ...
        for analysis in analysis_list:
            ...
            analysis_container_path = "{}/{}".format(
                project_id, parse.quote(analysis.value.title)
            )
        ...
        for global_model in global_model_list:
            ...
            global_model_container_path = "{}/{}".format(
                hyb_sim_container_path, parse.quote(global_model.value.title)
            )
            print("\tGlobal Model " + global_model.value.title)
            hyb_sim_map["fedora_mapping"]["generated"].append(
                "Global Model: {}".format(global_model.value.title)
            )
            ...
            global_model_map = {
                ...
                "fedora_mapping": {
                    **format_global_modal(global_model),
                    "wasGeneratedBy": "Hybrid Simulation: {}".format(hyb_sim_doi),
                },
            }
            ...
            coordinator_list = filter(
                ...
                and has_associations(coordinator, [global_model, hyb_sim]),
                getattr(doc, "coordinators", []),
            )
            for coordinator in coordinator_list:
                # Do stuff with coordinator.
                coordinator_container_path = "{}/{}".format(
                    global_model_container_path, parse.quote(coordinator.value.title)
                )
                print("\t\tCoordinator " + coordinator.value.title)
                hyb_sim_map["fedora_mapping"]["generated"].append(
                    "Coordinator: {}".format(coordinator.value.title)
                )
                ...
                coordinator_map = {
                    ...
                    "fedora_mapping": {
                        **format_coordinator(coordinator),
                        "wasGeneratedBy": "Hybrid Simulation: {}".format(hyb_sim_doi),
                        "wasInfluencedBy": "Global Model: {}".format(
                            global_model.value.title
                        ),
                    },
                }

                coordinator_output_list = filter(
                    lambda coordinator_output: coordinator.uuid
                    in coordinator_output.value.coordinators
                    and has_associations(
                        coordinator_output, [hyb_sim, global_model, coordinator]
                    ),
                    getattr(doc, "coordinator_outputs", []),
                )
                for coordinator_output in coordinator_output_list:
                    # Do stuff with coordinator output.
                    coordinator_output_container_path = "{}/{}".format(
                        coordinator_container_path,
                        parse.quote(coordinator_output.value.title),
                    )
                    print("\t\t\tCoordinator Output " + coordinator_output.value.title)
                    hyb_sim_map["fedora_mapping"]["generated"].append(
                        "Coordinator Output: {}".format(coordinator_output.value.title)
                    )
                    ...
                    coordinator_output_map = {
                        ...
                        "fedora_mapping": {
                            **format_coordinator_output(coordinator_output),
                            "wasGeneratedBy": "Hybrid Simulation: {}".format(
                                hyb_sim_doi
                            ),
                            "wasInfluencedBy": "Global Model: {}".format(
                                global_model.value.title
                            ),
                            "wasDerivedFrom": "Coordinator: {}".format(
                ...
                sim_sub_list = filter(
                    ...
                    and has_associations(sim_sub, [hyb_sim, global_model, coordinator]),
                    getattr(doc, "sim_substructures", []),
                )
                for sim_sub in sim_sub_list:
                    # Do stuff with sim substructure.
                    sim_sub_container_path = "{}/{}".format(
                        coordinator_container_path, parse.quote(sim_sub.value.title)
                    )
                    print("\t\t\tSimulation Substructure " + sim_sub.value.title)
                    hyb_sim_map["fedora_mapping"]["generated"].append(
                        "Simulation Substructure: {}".format(sim_sub.value.title)
                    )
                    ...
                    sim_sub_map = {
                        ...
                        "fedora_mapping": {
                            **format_sim_substructure(sim_sub),
                            "wasGeneratedBy": "Hybrid Simulation: {}".format(
                                hyb_sim_doi
                            ),
                            "wasInfluencedBy": "Global Model: {}".format(
                            ...
                            "wasInfluencedBy": "Coordinator: {}".format(
                                coordinator.value.title
                            ),
                        },
                    }
                    ...
                    sim_out_list = filter(
                        ...
                        and has_associations(sim_out, [hyb_sim, global_model]),
                        getattr(doc, "sim_outputs", []),
                    )
                    for sim_out in sim_out_list:
                        # Do stuff with sim output.
                        sim_out_container_path = "{}/{}".format(
                            sim_sub_container_path, parse.quote(sim_out.value.title)
                        )
                        print("\t\t\t\tSimulation Output " + sim_out.value.title)
                        hyb_sim_map["fedora_mapping"]["generated"].append(
                            "Simulation Output: {}".format(sim_sub.value.title)
                        )
                        ...
                        sim_out_map = {
                            ...
                            "fedora_mapping": {
                                **format_sim_output(sim_out),
                                "wasDerivedFrom": "Simulation Substructure: {}".format(
                                    sim_sub.value.title
                                ),
                            },
                        }
                ...
                exp_sub_list = filter(
                    ...
                    and has_associations(exp_sub, [hyb_sim, global_model]),
                    getattr(doc, "exp_substructures", []),
                )
                for exp_sub in exp_sub_list:
                    # Do stuff with experimental substructure.
                    exp_sub_container_path = "{}/{}".format(
                        coordinator_container_path, parse.quote(exp_sub.value.title)
                    )
                    print("\t\t\tExperimental Substructure " + exp_sub.value.title)
                    hyb_sim_map["fedora_mapping"]["generated"].append(
                        "Experimental Substructure: {}".format(exp_sub.value.title)
                    )
                    ...
                    exp_sub_map = {
                        ...
                        "fedora_mapping": {
                            **format_exp_substructure(exp_sub),
                            "wasGeneratedBy": "Hybrid Simulation: {}".format(
                                hyb_sim_doi
                            ),
                            "wasInfluencedBy": "Global Model: {}".format(
                    ...
                    exp_out_list = filter(
                        ...
                        and has_associations(exp_out, [hyb_sim, global_model]),
                        getattr(doc, "exp_outputs", []),
                    )
                    for exp_out in exp_out_list:
                        # Do stuff with experimental output.
                        exp_out_container_path = "{}/{}".format(
                            exp_sub_container_path, parse.quote(exp_out.value.title)
                        )
                        print("\t\t\t\tExperimental Output " + exp_out.value.title)
                        hyb_sim_map["fedora_mapping"]["generated"].append(
                            "Experimental Output: {}".format(exp_sub.value.title)
                        )
                        ...
                        exp_out_map = {
                            ...
                            "fedora_mapping": {
                                **format_exp_output(exp_out),
                                "wasDerivedFrom": "Experimental Substructure: {}".format(
                                    exp_sub.value.title
                                ),
                            },
                        }
...
def format_hyb_sim(hyb_sim):
    ...
    try:
        ...
        creator = list(
            map(lambda author: "{lname}, {fname}".format(**author.to_dict()), authors)
        )
    except AttributeError:
        creator = list(
            map(lambda username: _get_user_by_username(hyb_sim, username), meta.authors)
        )
