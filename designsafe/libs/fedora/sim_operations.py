    ...
    for sim in sims_list:
        ...
        for report in reports:
            ...
            report_container_path = "{}/{}".format(
                sim_container_path, parse.quote(report.value.title)
            )
            print("\treport " + report.value.title)
            sim_map["fedora_mapping"]["generated"].append(
        ...
        for analysis in analysis_list:
            ...
            analysis_container_path = "{}/{}".format(
                sim_container_path, parse.quote(analysis.value.title)
            )
        ...
        for model in models:
            # Do stuff with model.
            model_container_path = "{}/{}".format(
                sim_container_path, parse.quote(model.value.title)
            )
            print("\tmodel " + model.value.title)
            sim_map["fedora_mapping"]["generated"].append(
                "Simulation Model: {}".format(model.value.title)
            )
            ...
            inputs = filter(
                ...
                and has_associations(input, [model, sim]),
                getattr(doc, "inputs", []),
            )

            for input in inputs:
                input_container_path = "{}/{}".format(
                    model_container_path, parse.quote(input.value.title)
                )
                print("\t\tinput " + input.value.title)
                sim_map["fedora_mapping"]["generated"].append(
                    "Simulation Input: {}".format(input.value.title)
                )
                input_map = {
                    "uuid": input.uuid,
                    ...
                    "fedora_mapping": {
                        **format_input(input),
                        "wasGeneratedBy": "Simulation: {}".format(sim_doi),
                        "wasDerivedFrom": "Simulation Model: {}".format(
                            model.value.title
                        ),
                        "Influence": [],
                    },
                ...
                outputs = filter(
                    ...
                    and has_associations(output, [input, model, sim]),
                    getattr(doc, "outputs", []),
                )

                for output in outputs:
                    output_container_path = "{}/{}".format(
                        input_container_path, parse.quote(output.value.title)
                    )
                    print("\t\t\toutput " + output.value.title)
                    sim_map["fedora_mapping"]["generated"].append(
                        "Simulation Input: {}".format(output.value.title)
                    )
                    input_map["fedora_mapping"]["Influence"].append(
                        "Simulation Output: {}".format(output.value.title)
                    )
                    output_map = {
                        "uuid": output.uuid,
...
def format_sim(sim):
    ...
    try:
        ...
        creator = list(
            map(lambda username: _get_user_by_username(sim, username), meta.authors)
        )
