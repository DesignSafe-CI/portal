...
FEDORA_CONTEXT = {
    "abstract": {"@id": "http://purl.org/dc/elements/1.1/abstract"},
    "available": {"@id": "http://purl.org/dc/elements/1.1/available"},
    "description": {"@id": "http://purl.org/dc/elements/1.1/description"},
    "title": {"@id": "http://purl.org/dc/elements/1.1/title"},
    "identifier": {"@id": "http://purl.org/dc/elements/1.1/identifier"},
    "subject": {"@id": "http://purl.org/dc/elements/1.1/subject"},
    "creator": {"@id": "http://purl.org/dc/elements/1.1/creator"},
    "coverage": {"@id": "http://purl.org/dc/elements/1.1/coverage"},
    "license": {"@id": "http://purl.org/dc/elements/1.1/license"},
    "contributor": {"@id": "http://purl.org/dc/elements/1.1/contributor"},
    "type": {"@id": "http://purl.org/dc/elements/1.1/type"},
    "issued": {"@id": "http://purl.org/dc/elements/1.1/issued"},
    "date": {"@id": "http://purl.org/dc/elements/1.1/date"},
    "hasVersion": {"@id": "http://purl.org/dc/elements/1.1/hasVersion"},
    "_created": {"@id": "http://purl.org/dc/elements/1.1/created"},
    "publisher": {"@id": "http://purl.org/dc/elements/1.1/publisher"},
    "references": {"@id": "http://purl.org/dc/elements/1.1/references"},
    "isReferencedBy": {"@id": "http://purl.org/dc/elements/1.1/isReferencedBy"},
    "relation": {"@id": "http://purl.org/dc/elements/1.1/relation"},
    "isPartOf": {"@id": "http://purl.org/dc/terms/isPartOf"},
    "hasPart": {"@id": "http://purl.org/dc/terms/hasPart"},
    "entity": {"@id": "https://www.w3.org/TR/2013/REC-prov-o-20130430/#Entity"},
    "wasAttributedTo": {
    ...
    "generated": {"@id": "https://www.w3.org/TR/2013/REC-prov-o-20130430/#generated"},
    "wasGeneratedBy": {
    ...
    "influenced": {"@id": "https://www.w3.org/TR/2013/REC-prov-o-20130430/#influenced"},
    "Influence": {"@id": "https://www.w3.org/TR/2013/REC-prov-o-20130430/#Influence"},
}
...
def fedora_get(project_id, relative_to_root=True):
    ...
    url = project_id
    if relative_to_root:
        url = "{}{}/{}".format(settings.FEDORA_URL, PUBLICATIONS_CONTAINER, project_id)

    request = requests.get(
        url,
        auth=(settings.FEDORA_USERNAME, settings.FEDORA_PASSWORD),
...
def fedora_post(container_path):
    ...
    """
    fc_url = "{}{}/{}".format(
        settings.FEDORA_URL, PUBLICATIONS_CONTAINER, container_path
    )
    try:
        request = requests.put(
            fc_url, auth=(settings.FEDORA_USERNAME, settings.FEDORA_PASSWORD)
        )
        request.raise_for_status()
...
def fedora_update(container_path, update_body={}):
    ...
    updated_data["@type"] = initial_data["@type"] + [
        "http://purl.org/dc/dcmitype/Dataset"
    ]
    request = requests.put(
        "{}{}/{}".format(settings.FEDORA_URL, PUBLICATIONS_CONTAINER, container_path),
        auth=(settings.FEDORA_USERNAME, settings.FEDORA_PASSWORD),
        headers=FEDORA_HEADERS,
...
def format_metadata_for_fedora(project_id, version=None):
    ...
    try:
        ...
        author_list = list(
            map(
                lambda member: "{}, {}".format(member.lname, member.fname), ordered_team
            )
        )
        ...
        try:
            ...
            author_list = list(
                map(
                    lambda member: "{}, {}".format(member.lname, member.fname),
                    ordered_team,
                )
            )
        except AttributeError:
    ...
    for expt in experiments_list:
        ...
        for mc in model_configs:
            ...
            mc_map = {
                ...
                "fedora_mapping": {
                    **format_model_config(mc),
                    "wasGeneratedBy": exp_doi,
                },
            }
            ...
            for sl in sensor_lists:
                ...
                sl_container_path = "{}/{}".format(
                    configs_container_path, parse.quote(sl.value.title)
                )
                ...
                sl_map = {
                    ...
                    "fedora_mapping": {
                        **format_sensor_info(sl),
                        "wasGeneratedBy": "Experiment: {}".format(exp_doi),
                        "wasDerivedFrom": "Model Configuration: {}".format(
                            mc.value.title
                        ),
                        "influenced": [],
                    },
                ...
                events = filter(
                    ...
                    and expt.uuid in event.associationIds
                    and mc.uuid in event.associationIds,
...
def format_experiment(expt):
    ...
    try:
        ...
        creator = list(
            map(lambda username: _get_user_by_username(expt, username), meta.authors)
        )
    ...
    for entity in walk_result:
        ...
        for file in file_objs:
            ...
            rel_path = os.path.join(
                parse.unquote(entity["container_path"]), file["path"].strip("/")
            )
            ...
            if file["type"] == "dir":
                for path in get_child_paths(file_path):
                    ...
                    tags = filter(
                        lambda t: t["path"].strip("/") == tag_path.strip("/"), file_tags
                    )
...
def generate_package(project_id):
    ...
    archive_path = "/corral-repl/tacc/NHERI/published/archives/{}.zip".format(
        project_id
    )
